...
def main():
    ...
    if "conversation_id" not in st.session_state:
        st.session_state.vote_disabled = True
    ...
    search_type = st.radio("Select search type:", ["Text", "Vector"], index=1)
    ...
    if st.button("Ask"):
        ...
        with st.spinner("Processing..."):
            ...
            answer_data = get_answer(user_input, model_choice, search_type.lower())
            end_time = time.time()
            print_log(f"Answer received in {end_time - start_time:.2f} seconds")
            st.success("Completed!")
            st.write(answer_data["answer"])
            st.session_state.vote_disabled = False
    ...
    with col1:
        if st.button("+1", disabled=st.session_state.vote_disabled):
